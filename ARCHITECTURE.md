...
- Claude Code OAuth authentication for Pro/Max accounts is handled via `CLAUDE_CODE_OAUTH_TOKEN` (long-lived static token generated by `claude setup-token` on the host) — no port forwarding or browser flow is required inside the container
...
- For Pro/Max OAuth: set `CLAUDE_CODE_OAUTH_TOKEN` to a long-lived token generated by `claude setup-token` on the host (~1 year validity); Claude Code reads it directly from the environment with no startup exchange required
