...
for cmd in bw docker jq; do
    if ! command -v "$cmd" &>/dev/null; then
        case "$cmd" in
            bw)     echo "ERROR: Bitwarden CLI not found. Install: brew install bitwarden-cli" >&2 ;;
            docker) echo "ERROR: Docker not found. Install Docker Desktop for Mac first." >&2 ;;
            jq)     echo "ERROR: jq not found. Install: brew install jq" >&2 ;;
        esac
        exit 1
    fi
done
