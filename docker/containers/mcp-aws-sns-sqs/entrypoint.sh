...
log_info()    { echo -e "${_B}[mcp-aws-sns-sqs]${_N} $*"; }
log_warn()    { echo -e "${_Y}[mcp-aws-sns-sqs] WARN:${_N} $*" >&2; }
log_success() { echo -e "${_G}[mcp-aws-sns-sqs] ✓${_N} $*"; }
...
validate_secrets() {
    if [ -z "${AWS_ACCESS_KEY_ID}" ] || [ -z "${AWS_SECRET_ACCESS_KEY}" ] || [ -z "${AWS_REGION}" ]; then
        log_warn "AWS credentials not set - mcp-aws-sns-sqs idle."
...
export AWS_SECRET_ACCESS_KEY="${AWS_SECRET_ACCESS_KEY}"
export AWS_REGION="${AWS_REGION}"
export ALLOW_MUTATING_OPERATIONS="true"
export FASTMCP_LOG_LEVEL="ERROR"
exec uvx awslabs.amazon-sns-sqs-mcp-server@latest
EOF
    chmod +x /tmp/mcp-runner.sh
    log_info "Starting AWS SNS/SQS MCP server..."
    exec supergateway --port 3010 --outputTransport streamableHttp --stateful \
