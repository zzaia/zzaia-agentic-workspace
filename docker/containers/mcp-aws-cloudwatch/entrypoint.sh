...
log_info()    { echo -e "${_B}[mcp-aws-cloudwatch]${_N} $*"; }
log_warn()    { echo -e "${_Y}[mcp-aws-cloudwatch] WARN:${_N} $*" >&2; }
log_success() { echo -e "${_G}[mcp-aws-cloudwatch] ✓${_N} $*"; }
...
validate_secrets() {
    if [ -z "${AWS_ACCESS_KEY_ID}" ] || [ -z "${AWS_SECRET_ACCESS_KEY}" ] || [ -z "${AWS_REGION}" ]; then
        log_warn "AWS credentials not set - mcp-aws-cloudwatch idle."
...
EOF
    ...
    log_info "Starting AWS CloudWatch MCP server..."
    exec supergateway --port 3011 --outputTransport streamableHttp --stateful \
