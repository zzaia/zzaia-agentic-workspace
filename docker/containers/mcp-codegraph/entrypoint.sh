    ...
    while read worktree_dir; do
        ...
        for branch_dir in "$worktree_dir"/*; do
            if [ -d "$branch_dir" ] && [ -f "$branch_dir/.git" ]; then
                ...
                if cgc index "$branch_dir" 2>&1; then
                    echo "Watching for changes: $branch_dir"
                    cgc watch --poll "$branch_dir" > /dev/null 2>&1 &
                else
                    echo "Warning: Failed to index $branch_dir"
                fi
            fi
        done
    done < <(find "$WORKSPACE_PATH" -maxdepth 2 -name "*.worktrees" -type d)
