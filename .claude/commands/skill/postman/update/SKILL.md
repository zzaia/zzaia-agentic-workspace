...
sequenceDiagram
    ...
    U->>C: /skill:postman:update --target <type> --id <id> --spec <payload>
    C->>W: Update resource
...
```
/skill:postman:update --target collection --id "collection-abc123" --spec '{"name":"Updated Tests","description":"New description"}'
```

```
/skill:postman:update --target environment --id "staging" --spec '{"variables":{"api_key":"new_token"}}' --description "Rotate API key in staging environment"
```

```
/skill:postman:update --target request --id "Get Users" --spec '{"url":"{{api_url}}/v2/users"}' --description "Update endpoint to v2 API"
```
