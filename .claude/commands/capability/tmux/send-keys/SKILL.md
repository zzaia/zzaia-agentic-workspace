...
sequenceDiagram
    ...
    U->>C: /capability:tmux:send-keys --name dev --keys "npm start"
...
```
/capability:tmux:send-keys --name dev --keys "npm start"
/capability:tmux:send-keys --name dev --keys "docker-compose up" --pane 0
/capability:tmux:send-keys --name build --keys "make test" --description "running tests in background"
/capability:tmux:send-keys --name monitor --keys "watch -n 1 'ps aux | grep node'" --pane 1
```
