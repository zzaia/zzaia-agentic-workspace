...
Five MCP connections are configured in `~/.config/opencode/config.json`:

| MCP | Interaction | How to Interact | Use When | Example Tools |
|---|---|---|---|---|
| **bifrost** | Bifrost Code Mode | `listToolFiles()` → `getToolDocs(name, fn)` → `executeToolCode(code)` — Python/Starlark sandbox; `result["key"]` syntax (not dot notation); no async/await; assign final output to `result` | Aggregated work tools: web research, DevOps tickets/PRs, API testing, cloud ops | tavily, azure_devops, azure_portal, postman, github, playwright, aws_api (AWS and Azure tools require credentials) |
